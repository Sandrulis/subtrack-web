-- Reģistrācija: saskarnes valoda no auth metadata (`interface_language_code`), ja kods ir `public.languages`.
-- Palaid pēc 148_users_registration_country.sql.
...
comment on function public.handle_new_user() is
  'Jauns auth.users → public.users; metadata: registration_country, billing_currency, interface_language_code (ja katalogā).';
