...
**Versija:** `0.4.42` (skatīt **[Izmaiņu žurnāls](#izmaiņu-žurnāls)**; **0.4.x** sākas ar **0.4.0** = agrāk žurnāla **0.3.54**; PWA – **[PWA (SubTrack)](#pwa-subtrack)**). Produkcija: **[Vercel un domēns](#vercel-un-produkcijas-domēns)** (`repazy.com`). Lietotājam redzamais nosaukums – **`system_settings.system_name`** (admin **`/admin/system`**).
...
| **OG / dalīšana** | Sākumlapa: **`og:title`** / **`og:image:alt`** = byline tulkojums (ne fiksēts LV); **`og:locale`** saskan ar UI lokāli. Pārbaude: [Facebook Sharing Debugger](https://developers.facebook.com/tools/debug/) pēc deploy. Sociālo robotu crawl bieži **bez** lietotāja valodas sīkdatnes – rāda noklusējuma / **`Accept-Language`** lokāli. |
...
| PWA / logo | [Serwist](https://serwist.pages.dev) (`serwist.config.js`, `app/sw.ts` → `public/sw.js`); instalācijas UX – **`components/pwa/*`**; logo – **`sharp`** + Storage **`brand`** (`lib/brand/process-logo.ts`). Skatīt **[PWA (SubTrack)](#pwa-subtrack)** |
...
- **Lappušu `<title>` (App Router)** - daudzos maršrutos **`generateMetadata`** izsauc **`getUiPhraseForRequest('meta.title.*')`** (`lib/ui/server-ui-phrases.ts`; tās pašas lokāļa izvēles kā layout), piem.: **`/admin/*`**, **`/login`**, **`/signup`**, **`/dashboard`**, **`/analytics`**, **`/settings`**, **aizmirstā parole / mainīt paroli**. **Sākumlapa `/`:** **`getBrandBylinePageTitle`** + Open Graph (**`lib/seo/landing-seo.ts`**, **`lib/seo/site-share-metadata.ts`**).
...
lib/seo/                      # `landing-seo.ts`, `site-share-metadata.ts` (`getBrandBylinePageTitle`, OG/Twitter), `search-crawl.ts`
...
lib/auth/                 # user-display, pro-trial-access, pro-plan-access, grant-pro-trial-session, is-admin, actions
...
database/supabase/        # Postgres + RLS (`001` … **`116`**); PWA **`067`–`070`**, logo **`071`–`075`**, drošība **`078`–`080`**, push **`081`–`082`**, family **`084`–`095`**, todos **`096`–`100`**, Pro trial **`107`–`116`**
...
3. **Supabase un ENV** – salīdzināt **`database/supabase/`** (līdz **`116_*`**: ģimenes **`084`–`095`** + **`093`**, Pro trial **`107`–`116`**, drošība **`078`–`080`**, u.c.) un **`supabase.env.template`** ar **`.env.local`**. **Pro trial:** **`107`–`116`** ( **`116`** – RPC tikai `service_role`). **Drošība:** **`078`**, **`079`**, **`080`**, **`022`**, **`023`**, **`015`**, **`016`**. **`SUPABASE_SERVICE_ROLE_KEY`** obligāts: signup, Pro trial RPC, VIP, cron, daļa family **`PATCH`**. **Cron Vercel:** `Authorization: Bearer $CRON_SECRET` (bez `?secret=`). **Opcija:** `UPSTASH_REDIS_REST_*` (globāls rate limit). **Auth panelī:** ieslēgt **Leaked password protection** (skat. **Supabase iestatīšana**). Pēc SQL: **`npm run security:check`**. Ja mainīts **`styles/subtrack.css`**: **`npm run css:split`**. Migrācijas: **Supabase iestatīšana**, **`npm run security:migration-checklist`**, **`security_check.md`**.
