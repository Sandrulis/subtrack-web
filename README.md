...
- **Native Android (Capacitor)** – Play Store gatavs **WebView** aplikācijas čaulis: ielādē produkciju no **`https://repazy.com/login`** (ne lokālo `public/`). Viesiem logo un „mājas” saites ved uz **`/login`**, ne uz sākumlapu (**`lib/capacitor/*`**, **`NavLanding`**); **PWA instalācijas banneris** appā **nerāda** (**`PwaInstallHost`**, **`install-prompt-capture.ts`**). Pilns ceļvedis: **[Capacitor (Android)](#capacitor-android)**.
