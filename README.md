...
**Versija:** `0.4.12` (skatīt **[Izmaiņu žurnāls](#izmaiņu-žurnāls)**; **0.4.x** sākas ar **0.4.0** = agrāk žurnāla **0.3.54**; PWA – **[PWA (repazy)](#pwa-repazy)**).
...
- **Ģimenes dalīšana** (`/family-sharing`) – tikai ja admin **`/admin/integrations`** ieslēdz **`family_sharing`**; saite lietotāja izvēlnē (**`session.family_sharing`**, **`components/nav-user-menu.tsx`**, karodziņš no **`SubtrackIntlProvider.integrations`**). Uzaicinājums pa e-pastu (lietotājam jāeksistē **`public.users`**), pieņemšana / atcelšana, krāsa partnera izdevumiem, slēdzis **„saskaitīt kopā”** kopsavilkumā. **`/dashboard`**: partnera abonementi **tikai lasāmi** (vieglā fona krāsa, birka „Kopīgots”), kalendārā krāsaina apmale; kopējā summa ar **\***, apakšā labajā – **„tikai mani izdevumi”**, ja slēdzis ieslēgts. UI platums un formas kā analītikā (**`stat-card`**, **`form-group`**, **`--app-shell-max`**). DB: **`084_family_sharing.sql`** (`family_sharing_links`, RLS, owner lasa partnera **`subscriptions`**). API: **`/api/family-sharing`**, **`/api/family-sharing/[id]`**. Kods: **`lib/family-sharing/*`**, **`lib/integrations/integration-enabled.ts`**, **`components/family-sharing/family-sharing-view.tsx`**, **`public/fs/js/dashboard.js`**. Tulkošanas **`085_*`** + **`fallback-phrases.ts`** (`translation_key` / `locale` / `value`). Uzaicinājumam nepieciešams **`SUPABASE_SERVICE_ROLE_KEY`** (e-pasta meklēšana).
...
app/api/family-sharing/   # ģimenes dalīšana: uzaicinājumi, PATCH (krāsa, combine, accept, revoke)
...
database/supabase/        # Postgres + RLS (`001` … `085` utt.; PWA **`067`–`070`**, logo **`071`–`075`**, drošība **`078`–`080`**, push **`081`–`082`**, family **`084`–`085`**)
...
3. **Supabase un ENV** – salīdzināt **`database/supabase/`** (līdz **`085_*`**, ģimenes dalīšana **`084`–`085`**) un **`supabase.env.template`** ar **`.env.local`**. **Drošība (2026-05):** **`078`**, **`079`**, **`080`**; **`SUPABASE_SERVICE_ROLE_KEY`** – signup, VIP, cron. Logo: **`071`–`074`**, **`072`**, **`077`**. PWA SQL: **`067`–`070`**, **`074`**. Pēc SQL: **`npm run security:smoke`**. Migrācijas secība – **Supabase iestatīšana** un **`npm run security:migration-checklist`**. PWA: **`npm run build`** (ģenerē **`public/sw.js`**) vai **`npm run dev`** – skatīt **[PWA](#pwa-repazy)**.
