...
**Versija:** `0.3.29` (skatīt lapas lejas daļā **[Izmaiņu žurnāls](#izmaiņu-žurnāls)**).
...
- **Globālais UI valodas slēdzis** – karoga poga augšējā joslā (`NavUiLanguageSwitcher`) + mobilajā apakšējā joslā; `subtrack_ui_locale` sīkdatne, `localStorage` prefs sinhronizācija, **`router.refresh()`**; valodu katalogs ar nosaukumiem **`getLanguagesCatalog()`** (`languages-catalog.ts` v2 kešs); tulkošanas **`042`**, **`fallback-phrases.ts`**; **`dash-alerts.js`** aizver paziņojumus atverot valodu.
