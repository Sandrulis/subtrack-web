...
**Versija:** `0.4.20` (skatīt **[Izmaiņu žurnāls](#izmaiņu-žurnāls)**; **0.4.x** sākas ar **0.4.0** = agrāk žurnāla **0.3.54**; PWA – **[PWA (SubTrack)](#pwa-subtrack)**).
...
- **Ģimenes dalīšana** (`/family-sharing`) – tikai ja admin **`/admin/integrations`** ieslēdz **`family_sharing`**; saite lietotāja izvēlnē (**`session.family_sharing`**, **`components/nav-user-menu.tsx`**, karodziņš no **`SubtrackIntlProvider.integrations`**). Uzaicinājums pa e-pastu (lietotājam jāeksistē **`public.users`**; POST iestata **`partner_user_id`**), pieņemšana / **noraidīšana** (uzaicinātais), atcelšana (**owner**), **pamest dalību** (**partner**). UI: **Saņemtie uzaicinājumi** (ikonpogas ar ielādes riņķi); **Tavi uzaicinājumi** (owner); **Dalība ar mani** (partner). Slēdzis **„saskaitīt kopā”** – **katram savs** (**`owner_combine_in_totals`** / **`partner_combine_in_totals`**, **`095`**): viens ieslēdz, otrs var atstāt izslēgtu; kopsumma un **\*** tikai tam, kur slēdzis ieslēgts. Tukšas sadaļas paslēptas. Augšējā joslā **paziņojumu zvans** arī šajā lapā: **`#subtrack-subs-bootstrap-json`** + **`FsNotifyI18nBootstrap`** (kavētie / šodien / gaidāmie – tikai **paši** ieraksti); ģimenes uzaicinājumi panelī (**`dash-alerts.js`**, **`#subtrack-family-sharing-bootstrap-json`**). **`/dashboard`**: kopīgotie ieraksti **tikai lasāmi**, kopsumma ar **\*** un **`viewerUserId`**. DB: **`084`–`095`**. API: **`GET /api/family-sharing`** (`viewerUserId`, `links` ar viewer-specific **`combineInTotals`**); **`PATCH`**: `accept`, `decline`, `revoke`, `leave`, `color`, `combineInTotals` (stāvokļa maiņas un lasīšana ar **`service_role`** pēc sesijas pārbaudes). Kods: **`lib/family-sharing/*`**, **`family-sharing-view.tsx`**, **`dashboard.js`**, **`dash-alerts.js`**, **`nav-dash.tsx`** (`reloadSubscriptionsFromBootstrap`). Tulkošanas **`085_*`**, **`database/translations_daily/2026-05-21.sql`**. **`SUPABASE_SERVICE_ROLE_KEY`** – uzaicinājuma e-pasts, lasīšana Vercel un uzticamas **`PATCH`** operācijas.
...
database/supabase/        # Postgres + RLS (`001` … `100` utt.; PWA **`067`–`070`**, logo **`071`–`075`**, drošība **`078`–`080`**, push **`081`–`082`**, family **`084`–`095`**, admin todos **`096`–`100`**)
...
4. **Pārbaude** – **`npm run lint`** un **`npm run build`** pēc lielākām izmaiņām; ikdienas **`npm run dev`**. Ja mainīta drošība/DB: **`npm run security:check`**. Mobilā: PWA banneris + **`/offline`** (**[PWA](#pwa-subtrack)**). Turbopack **`CssSyntaxError`** – dzēst **`.next`**, restartēt dev (žurnāls **0.3.8**).
