...
**Versija:** `0.5.29` (skatīt **[Izmaiņu žurnāls](#izmaiņu-žurnāls)**; **0.4.x** sākas ar **0.4.0** = agrāk žurnāla **0.3.54**; PWA – **[PWA (SubTrack)](#pwa-subtrack)**). Produkcija: **[Vercel un domēns](#vercel-un-produkcijas-domēns)** (`repazy.com`). Lietotājam redzamais nosaukums – **`system_settings.system_name`** (admin **`/admin/system`**).
...
**Vercel Cron (piemērs):** visiem – `Authorization: Bearer $CRON_SECRET` ( **`lib/security/cron-auth.ts`** ; bez `?secret=` URL). Bieži **`0 * * * *`** (katru stundu) due-today / weekly / trial.
...
**SQL:** **`123_email_notification_preferences.sql`**, **`124_site_translations_email_cron_notifications.sql`**, **`127_*`** (admin cron UI), **`129_*`** (teksti bez kavēto cron).
...
lib/emails/               # admin šabloni, Resend (signup/reset/cron: due-today/weekly/trial)
...
app/api/cron/             # due-today, weekly-summary, trial-ending, payment-push (CRON_SECRET)
...
3. **Supabase un ENV** – salīdzināt **`database/supabase/`** (līdz **`154_*`**: blogs **`153`–`154`**, atsauksmes **`152`**, feedback **`151`**, ieteikumi **`150`**, atbalsta e-pasts **`149`**, reģistrācijas valsts **`148`**, admin **`last_seen`** **`145`–`147`**, u.c.) un **`supabase.env.template`** ar **`.env.local`**. **Blogs:** **`153_blog_posts.sql`**, **`154_blog_storage.sql`**; attēlu augšupielādei – **`SUPABASE_SERVICE_ROLE_KEY`** (kā logo **`072`**). **Pro trial:** **`107`–`116`**. **Drošība:** **`078`–`080`**, **`022`**, **`023`**. **`SUPABASE_SERVICE_ROLE_KEY`** obligāts: signup/confirm e-pasti, Pro trial RPC, VIP, cron, admin user delete, daļa family **`PATCH`**, blog/storage (ieteicams). **Resend:** `RESEND_API_KEY`, `EMAIL_FROM`. **Cron:** `CRON_SECRET`. Pēc SQL: **`npm run security:check`**. Ja mainīts **`styles/subtrack.css`**: **`npm run css:split`**. Tulkošanas: **`database/translations_daily/2026-05-29.sql`** (t.sk. **`admin.blog.*`**, **`blog.*`**, **`legal.footer.blog`**). Migrācijas: **Supabase iestatīšana**, **`npm run security:migration-checklist`**, **`security_check.md`**.
