...
- **Native Android (Capacitor)** – Play Store gatavs **WebView** čaulis: **`https://repazy.com/login?native_shell=1`** (**`capacitor.config.ts`**). Viesiem logo / „mājas” → **`/login`**; **iziet** → **`/login?native_shell=1`** (ne `/`); **PWA instalācijas banneris** un **service worker** appā **izslēgti** (lai Capacitor plugini, piem. **launcher badge**, strādātu); **ikonas skaitlis** = tas pats kā zvans (**`@capawesome/capacitor-badge`**, **`lib/pwa/app-badge.ts`**). **Atvēršanā** – zaļš splash + **`CapacitorNativeAppLoading`** (logo no **`/admin/system`**, progress josla, **`app.page_loading`**); **Android 13+** lūdz **paziņojumu** atļauju (**`request-native-permissions.ts`**). **Logo:** panelis/PWA/favicon un atsevišķs **topbar** – **`/admin/system`**; **launcher ikona** (mipmap) – **`npx @capacitor/assets generate`** vai manuāli **`android/.../mipmap-*`**. Skat. **[Capacitor (Android)](#capacitor-android)**.
...
**PWA instalācijas banneris** – virs apakšējās navigācijas (`z-index` **185**); tikai **≤960 px** un ceļos **`/dashboard`**, **`/analytics`**, **`/settings`** (skatīt **[PWA](#pwa-subtrack)**). **Atbalsts / ieteikumi / atsauksmes** – footer teksta saites (skat. **Galvenās iespējas**), ne apakšējā pill.
...
| **Veiktspēja** | Custom domēns vs **`*.vercel.app`** parasti **nemaina** lab Lighthouse skaitļus; svarīgāk deploy, JS/CSS apjoms un mobilais **LCP** / **Speed Index**. **`/`** → **`styles/landing.css`** (~154 KB); pārējās lapas → **`styles/subtrack-app.bundle.css`** (~211 KB); avots **`styles/subtrack.css`** (~251 KB). **`css:split`**: **`styles/modules/*`** + bundle bez **`@import`**. App bundle ietver arī **`demo-app.css`** (`/demo/*` banneris un badges). **`scripts/split-landing-css.mjs`** griež pēc **sekciju komentāru marķieriem** `subtrack.css` (ne fiksētām rindām) un pārbauda PostCSS; pēc **`subtrack.css`** – **`npm run css:split`** (citādi **`landing.css`** **`Unexpected }`** – **neredigēt** `landing.css` ar roku). **`.cursor/rules/subtrack-css-split.mdc`**. **0.4.24** RSC; **0.4.26** FA nebloķējošs. **Panelis** – skat. **[Panelis un Lighthouse](#panelis-un-lighthouse-mobilais)**. Pārbaude: [PageSpeed Insights](https://pagespeed.web.dev/) uz **`/dashboard`** (ne tikai `/`) production URL. |
...
Pēc **0.5.18** (`AppPageContentGate`) un lielāka paneļa update (**0.5.16**–**0.6.x**) mobilajā Lighthouse (**Slow 4G**, Moto G) **`/dashboard`** Performance bieži **~80–85** (oranžs), ne **~90+** (zaļš) kā agrāk. **Accessibility / Best Practices / SEO** parasti paliek augsti; kritiskākās metrikas ir **Speed Index** un **LCP**, ne **TBT** vai **CLS**.
...
**Citi faktori (0.6.5):** mobilajā **`content-visibility: auto`** abonementu sarakstam (**`styles/subtrack.css`**); FA **`prefetch`** nevis **`preload`** (**`font-awesome-deferred-head.tsx`**); **Sentry Replay** – lazy **`import('@sentry/browser')`** pēc **`requestIdleCallback`**, **`replaysSessionSampleRate: 0.01`** (**`instrumentation-client.ts`**). **`subtrack-app.bundle.css`** (~**211 KB**) joprojām ietver ne-panela moduļus (demo u.c.) – pilns CSS split nav šajā izlaidumā.
...
| Kas | Kur / vērtība |
|-----|----------------|
| Konfigurācija | **`capacitor.config.ts`** – `appId` **`com.repazy.app`**, `appName` **repazy**, `webDir` **`public`**, **`server.url`** **`https://repazy.com/login?native_shell=1`**, `plugins.Badge` (`persist: true`, `autoClear: false`), SplashScreen (**`launchAutoHide: false`**, **`#00a38d`**, **`android/drawable/splash.xml`**) |
...
| Ielādes ekrāns | **`@capacitor/splash-screen`** + **`CapacitorNativeAppLoading`** – logo no admin, **`app.page_loading`**, progress; paslēpj pēc **`subtrack:native-shell-ready`** / satura |
...
components/authed/          # authed-footer-action-links, authed-nav-overlays-provider (support + suggestions + feedback modāļi)
...
- **Win-back e-pasti** – cron **`GET /api/cron/win-back-7d-emails`**, **`GET /api/cron/win-back-30d-emails`** (neaktīvs tieši 7 / 30 kalendāra dienas, `users.last_seen`, 09:00 lietotāja TZ). Admin šabloni **`win_back_7d`**, **`win_back_30d`** (`/admin/email-design`). Prefs **`win_back`** (`/email-notifications`). SQL **`155_*`**, tulkošanas **`database/translations_daily/2026-05-29.sql`**, **`lib/cron/run-win-back-emails.ts`**.
